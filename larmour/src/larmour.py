...
def main():
ps = ArgumeGntParser(prog='larmour',
...
if args.query is not None:
    ...
    traces = []
    traces.append({'x':b,'y':lp,'yaxis':'y1','mode':'lines','name':r'1H'})
    traces.append({'x':b,'y':le,'yaxis':'y2','mode':'lines','name':r'e-'})
    layout['yaxis1'] = {'side':'left', 'anchor':'free','position':0,'range':[0,mlp*1.1], 
        'title':r'1H Larmour (MHz)','exponentformat':'power','tickformat':".4g"}
    layout['yaxis2'] = {'side':'right', 'anchor':'free','position':1,'range':[-0.1*mle,mle],
        'overlaying':'y1','title':'e- Larmour (GHz)','exponentformat':'power',
        'tickformat':".4g"}
    pio.write_html({'data':traces,'layout':layout},'freqPE.html',auto_open=True)
